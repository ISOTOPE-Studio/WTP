		...
		if (cmd.getName().equalsIgnoreCase("wtp")) {
			if (args.length > 0 && !args[0].equals("help") && sender instanceof Player) {
				...
				if (args[0].equals("about")) {
					...
					sender.sendMessage(new StringBuilder().append(ChatColor.RED).append("未知命令，输入/wtp查看帮助").toString());
					return true;
				}
			} else { // Help Menu
				sender.sendMessage(
				...
				sender.sendMessage(
						new StringBuilder().append(ChatColor.GOLD).append("/wlist [页数]").append(ChatColor.GRAY)
				...
				sender.sendMessage(new StringBuilder().append(ChatColor.GOLD).append("/wtp relocate <地标名字>")
						...
						.append("花费" + WTPConfig.relocationFee + "更改公共地标为当前位置").toString());
				sender.sendMessage(
						new StringBuilder().append(ChatColor.GOLD).append("/wtp about").append(ChatColor.GRAY)
